...
public class SUcConGameSDk implements ISGameSDK {
    ...
    private static SUcConGameSDk sSUcConGameSDk;
    private InitCallback mSGameInitCallback;
    private LogoutCallback mLogoutCallback;


    public static SUcConGameSDk getImpl() {
        if (sSUcConGameSDk == null) {
            synchronized (SUcConGameSDk.class) {
                if (sSUcConGameSDk == null) {
                    sSUcConGameSDk = new SUcConGameSDk();
                }
            }
        }
        return sSUcConGameSDk;
    }
    ...
    public void init(Context context, Config config, InitCallback callback) {
        ...
        if (TextUtils.isEmpty(appId)) {
            ToastUtil.show(TAG, "初始化失败，请在String.xml文件配置游戏appId 15  code:00035");
            GameConError error = new GameConError();
            error.setCode("00035");
            error.setMessage("初始化失败，请在String.xml文件配置游戏appId 16");
...
//        gameParamInfo.setGameId(UCSdkConfig.gameId);
    ...
    SDKEventReceiver receiver = new SDKEventReceiver() {  //所有接收SDK事件的对象必须从SDKEventReceiver派⽣
        ...
        private void onInitSucc() {  //初始化账户成功
            Log.d(TAG, "initSGameSDK initGame: success 2");
...
//            AccountInfo.instance().setSid(sid);
        ...
        private void onLoginFailed(String desc) {  //登录失败
            if (mLoginCallback != null) {
                GameConError loginError = new GameConError();
                loginError.setMessage(desc);
                loginError.setCode(String.valueOf(GameConError.LOGIN_ERROR));
        ...
        private void onLogoutFailed() { //注销失败
            if (mLogoutCallback != null) {
                GameConError loginError = new GameConError();
                loginError.setCode(String.valueOf(GameConError.LOGOUT_ERROR));
