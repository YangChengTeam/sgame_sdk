...
public class SAdSDK implements ISGameSDK {
    ...
    public void init(Context context, Config config, InitCallback callback) {
        SOppoAdSDk.getImpl().init(context, config, callback);
    }
    ...
    public void showAd(Context context, AdType type, AdCallback callback, ViewGroup viewGroup) {
        if (type != AdType.SPLASH && type != AdType.VIDEO_REWARD && type != AdType.VIDEO_REWARD_V && Build.VERSION.SDK_INT >= 23) {  //闪屏类没有下载类广告，不需要用户授权动态权限
